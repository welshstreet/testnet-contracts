...
export function emissionRewards(
    rewards: number,
    totalLp: number,
    sender: any,
    disp: boolean = false
    ...
    const test = simnet.callPublicFn(
        "street-rewards",
        "emission-rewards",
        [Cl.uint(rewards), Cl.uint(totalLp)],
        actualSender
    );
    ...
    if (test.result.type === 'err') {
        ...
        expect(test.result).toEqual(Cl.error(Cl.uint(errorCode)));
        if (disp) {
            console.log(`☑️ Emission rewards failed with error: ${errorCode}`);
        }
        return false;
    }
    ...
    if (disp && test.result.type === 'ok') {
        console.log(`✅ Emission rewards successful: ${rewards} STREET distributed to ${totalLp} LP`);
    }
    return true;
}
...
export function getCleanupRewards(
    ...
    const test = simnet.callReadOnlyFn("street-rewards", "get-cleanup-rewards", [], sender);
...
export function updateBurnRewards(
    ...
    if (sender != deployer) {
        expect(test.result).toEqual(Cl.error(Cl.uint(903))); // ERR_NOT_AUTHORIZED
        if (disp) {
