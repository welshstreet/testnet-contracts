import { Cl } from '@stacks/transactions';
import { expect } from 'vitest';

export type ContractName = 'credit-token' | 'street-token';

export function setTokenUri(
    contract: ContractName,
    tokenUri: string,
    sender: string,
    disp: boolean = false
    ) {
    if (disp) {
        console.log(`\n=== setTokenUri: ${contract} ===`);
        console.log(`Sender: ${sender}`);
        console.log(`New Token URI: ${tokenUri}`);
    }
    const result = simnet.callPublicFn(
        contract,
        'set-token-uri',
        [Cl.stringUtf8(tokenUri)],
        sender
    );
    if (disp) {
        console.log(`Result type: ${result.result.type}`);
    }
    if (result.result.type === 'err') {
        const errorValue = Number((result.result as any).value.value);
        if (disp) {
            console.log(`☑️ Token URI change failed with error: ${errorValue}`);
        }
        expect(result.result).toEqual(Cl.error(Cl.uint(errorValue)));
        return result;
    }
    expect(result.result).toEqual(Cl.ok(Cl.bool(true)));
    if (disp) {
        console.log('✅ Token URI set successfully');
    }
    return result;
}